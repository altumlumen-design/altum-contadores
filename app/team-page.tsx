...
export default function TeamPage() {
  return <SiteChrome active="Equipo">
    ...
    <section className="profiles-section inner-shell" id="perfiles"><div className="page-section-heading"><div className="section-kicker"><span>01</span><p>Perfiles</p></div><h2>Las personas detrás de cada perspectiva.</h2><p>Conoce a quien dirige el estudio y las especialidades que se integran de acuerdo con cada necesidad empresarial.</p></div><div className="profiles-grid">{teamProfiles.map((profile, index) => <article className={index === 0 ? 'profile-card featured' : 'profile-card'} key={`${profile.role}-${index}`}><div className={profile.image ? 'profile-portrait has-photo' : 'profile-portrait'}>{profile.image ? <img src={assetPath(profile.image)} alt={`Retrato de ${profile.name}`} width={880} height={1100} /> : <UserRound aria-hidden="true" />}<span>{profile.initials}</span></div><div className="profile-status">{profile.status}</div><h3>{profile.name}</h3><p className="profile-role">{profile.role}</p><p className="profile-description">{profile.description}</p><ul>{profile.focus.map((item) => <li key={item}>{item}</li>)}</ul></article>)}</div></section>
